        ...
        <div className="md:w-[85%] m-auto grid grid-cols-1 gap-4 pb-8 md:pt-4">
            {quoteText.map((item, index) => (
                <blockquote key={index} className="text-xl text-white text-center m-auto px-8 py-4">
                    <p className="italic text-black dark:text-white">&quot;{item.quote}&quot;</p>
                    <p className="text-black dark:text-white">- {item.author}, <a href={item.link} target="_blank" className="text-rhino-purple hover:brightness-50 transition-all">{item.source}</a></p>
                </blockquote>
            ))}
        </div>
    );
