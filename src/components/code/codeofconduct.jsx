...
function CodeOfConduct() {
    return(
        <div className="md:w-[85%] w-full mx-auto md:p-0 p-4 mt-4">
            ...
            {CoC.map((item, index) => (
                <div key={index} className="text-white text-xl py-4">
                    <h2 className="text-3xl text-white py-4">{item.heading}</h2>
                    {item.content.map((paragraph, i) => (
                        <p className="text-white text-lg py-2" key={i}>{paragraph}</p>
                    ))}
                </div>
            ))}
        </div>
